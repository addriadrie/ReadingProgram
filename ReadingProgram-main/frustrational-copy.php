<?php
    ...
    require_once 'frustrational_config.php';
    $dbHelper = new DatabaseHelper();

    // Debug: Check what's in the session
    error_log("DEBUG: Session contents: " . print_r($_SESSION, true));

    // Check if we have GMRT results - if not, redirect back to test.php
    if (!isset($_SESSION['gmrt_test_results'])) {
        error_log("ERROR: No GMRT results found. Redirecting to test.php");
        header('Location: test.php');
        exit;
    }

    // Initialize frustrational session variables if not set
    if (!isset($_SESSION['frustrational_initialized'])) {
        ...
        $_SESSION['frustrational_initialized'] = true;
        error_log("DEBUG: Frustrational session initialized");
    }
        ...
        if (isset($_POST['action'])) {
            switch ($_POST['action']) {
                ...
                case 'submit_posttest':
                    ...
                    $_SESSION['posttest_score'] = calculateScore($_SESSION['posttest_answers'], $frustrationalContent[0]['comprehension']);
                    ...
                    if ($gmrtResults) {
                        ...
                        // Save ALL results to database (GMRT + frustrational reading activity)
                        $saveResult = saveResultsToDatabase(
                            ...
                            // frustrational reading activity results (from frustrational.php)
                ...
                case 'complete_reset':
                    // Complete reset - destroy all session data and redirect to test.php
                    session_unset();
                    session_destroy();
                    header('Location: test.php');
                    exit;
                ...
                case 'restart_frustrational':
                    // Only restart frustrational part, keep GMRT results
                    $gmrtResults = $_SESSION['gmrt_test_results']; // Save GMRT results
                    
                    // Clear only frustrational-specific session variables
                    unset($_SESSION['test_stage']);
                    unset($_SESSION['pretest_answers']);
                    unset($_SESSION['posttest_answers']);
                    unset($_SESSION['activity_answers']);
                    unset($_SESSION['pretest_score']);
                    unset($_SESSION['posttest_score']);
                    unset($_SESSION['activity_score']);
                    unset($_SESSION['selected_activity']);
                    unset($_SESSION['frustrational_initialized']);
                    unset($_SESSION['pretest_start_time']);
                    unset($_SESSION['activity_start_time']);
                    unset($_SESSION['posttest_start_time']);
                    
                    // Restore GMRT results
                    $_SESSION['gmrt_test_results'] = $gmrtResults;
                    
                    // Redirect to self to reinitialize
                    header('Location: frustrational-copy.php');
                    exit;
                    break;
            }
        }
    ...
    function saveResultsToDatabase(
        ...
        $data = [
            ...
            // frustrational reading activity results
...
<body>
    <div class="container">
        ...
        <div style="background: #f0f0f0; padding: 10px; margin: 10px 0; border: 1px solid #ccc;">
            ...
            <p><strong>Test Stage:</strong> <?php echo $_SESSION['test_stage'] ?? 'Not set'; ?></p>
            <p><strong>GMRT Results:</strong> <?php echo isset($_SESSION['gmrt_test_results']) ? 'Present' : 'Missing'; ?></p>
            <p><strong>Frustrational Initialized:</strong> <?php echo isset($_SESSION['frustrational_initialized']) ? 'Yes' : 'No'; ?></p>
        </div>
        <?php endif; ?>
        ...
        switch ($_SESSION['test_stage']) {
            case 'start':
                ?>
                <h1>Frustrational Reading Level Assessment</h1>
                <div class="instructions">
                    <h3>Welcome to the Frustrational Reading Level Test</h3>
                    <p>Based on your previous assessment, you've been identified as a frustrational reader. This specialized test will help us better understand your reading abilities and provide targeted support.</p>
           ...
           case 'posttest_combined':
                $posttest = $frustrationalContent[0];
                ?>
            ...
            default:
                ...
                <div class="alert alert-danger">
                    <h3>Error: Unknown Test Stage</h3>
                    <p>There was an error with the test state. Please restart the assessment.</p>
                </div>
                <form method="post">
                    <button type="submit" name="action" value="complete_reset" class="btn btn-danger">Restart Assessment</button>
                </form>
                <?php
                break;
        }
