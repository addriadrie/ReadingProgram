<?php
    ...
    // Add this function to save test results to session
    function saveTestResultsToSession($vocabScore, $speedScore, $comprehensionScore, $vocabAnswers, $speedAnswers, $comprehensionAnswers) {
    $_SESSION['test_results'] = [
        'vocab_score' => $vocabScore,
        'speed_score' => $speedScore, 
        'comprehension_score' => $comprehensionScore,
        'vocab_answers' => $vocabAnswers,
        'speed_answers' => $speedAnswers,
        'comprehension_answers' => $comprehensionAnswers,
        'overall_score' => $vocabScore + $speedScore + $comprehensionScore,
...
<body>
    ...
    <script>
        ...
        function continueToReading(url) {
            // Send test results to PHP session before redirecting
            const scoreData = scoreCompleteTest();
            
            const testData = {
                // Store test.php results with clear naming
                gmrt_vocab_score: scoreData.vocab.correct,
                gmrt_speed_score: scoreData.speed.correct,
                gmrt_comprehension_score: scoreData.comprehension.correct,
                gmrt_vocab_answers: vocabAnswers,
                gmrt_speed_answers: speedAnswers,
                gmrt_comprehension_answers: Object.fromEntries(new FormData(document.getElementById('comprehensionTest'))),
                gmrt_vocab_total: scoreData.vocab.total,
                gmrt_speed_total: scoreData.speed.total,
                gmrt_comprehension_total: scoreData.comprehension.total
            };
            ...
            .then(data => {
                if (data.success) {
                    ...
                    alert('Warning: Test results may not be saved properly');
                    window.location.href = url;
                }
            })
            .catch(error => {
        ...
        function determineReaderType(vocabPercentage, comprehensionPercentage, overallPercentage) {
            if (vocabPercentage >= 95 && comprehensionPercentage >= 90 && overallPercentage >= 95) {
                ...
                return {
                    ...
                    redirectUrl: 'frustrational-copy.php',
                    color: '#dc3545', // Red
                    icon: '💪'
                };
            }
